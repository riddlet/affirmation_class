...
###### Justifications (others w/varying certainty) ########### (this is unfinished)

df.just <- read.csv('output/nnprobs_just_cert.csv', stringsAsFactors = F)

df.just %>%
  group_by(sentence_num, class, val, just, mod) %>%
  mutate(word_num = row_number(),
         prob_sum = cumsum(prob_shift),
         keyword = max(word_num)) %>%
  ungroup() %>%
  filter(word==mod | word==just) -> plot.dat
...
plot.dat %<>% 
  mutate(class = fct_recode(class,
                            ...
                            'Control-Female' = 'control_f',
                            'Control-Male' = 'control_m')) %>%
  mutate(word_type = word==just) %>%
  mutate(word_type = recode(as.character(word_type), 
                            'TRUE'='justification', 'FALSE'='modifier')) %>%
  select(class, mod_type, prob_shift, sentence_num, val, mod, word_type) %>%
  spread(word_type, prob_shift)
  
  select(key, class, mod_type, sentence_num, val, prob_shift) %>%
  spread(key, prob_shift)

names(plot.dat)[c(5,6)] <- c('justification_shift', 'certainty_shift')

  group_by(sentence_num, class, val, just, mod) %>%
  

ggplot(plot.dat, aes(x=))
